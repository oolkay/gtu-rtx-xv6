...
/*   Updated: 2024/03/09 15:24:09 by acepni           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if ((map[i][j + 1] && ft_strchr("0NSEW", map[i][j + 1])) || (j - 1 > 0 && map[i][j - 1] && ft_strchr("0NSEW", map[i][j - 1])) || (map[i + 1] && map[i + 1][j] && ft_strchr("0NSEW", map[i + 1][j])) || (i - 1 > 0 && map[i - 1] && map[i - 1][j] && ft_strchr("0NSEW", map[i - 1][j])))
		return (1);
	return (0);
}
...
static int ft_space_surr_check(char **map, int i, int j)
{
	if (!map[i][j + 1] || (map[i][j + 1] && map[i][j + 1] == ' '))
		...
		return (1);
	else if (!map[i + 1] || (map[i + 1] && j >= (int)ft_strlen(map[i + 1])) || (map[i + 1] && j < (int)ft_strlen(map[i + 1]) && map[i + 1][j] == ' '))
		return (1);
	else if (i - 1 < 0 || (i - 1 >= 0 && j >= (int)ft_strlen(map[i - 1])) || (i - 1 >= 0 && j < (int)ft_strlen(map[i - 1]) && map[i - 1][j] == ' '))
		return (1);
	return (0);
}
